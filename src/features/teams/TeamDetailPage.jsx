...
function ResumenTab({ presidentName, photo, coachName, managerPhoto, honours, starters }) {
  return <div className="club-resumen-tab">
    <div className="club-resumen-left">
      ...
      <div className="club-resumen-honours-mini"><small>PALMARÉS</small><b className="club-resumen-honours-count">{honours.length} TÍTULO{honours.length === 1 ? '' : 'S'}</b>{honours.length ? <ul className="club-resumen-honours-list">{honours.map(item => <li key={item.id}><span aria-hidden="true">★</span><span>{item.name}{item.season && ` (${item.season})`}</span></li>)}</ul> : <p className="empty-copy">SIN TÍTULOS.</p>}</div>
    </div>
