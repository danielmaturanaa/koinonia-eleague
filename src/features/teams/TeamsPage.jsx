...
export function TeamsPage({ teams, loading, onChoose, onChanged }) {
  ...
  return <main className="newspaper teams-page"><section className={`team-list-paper ${showForm ? 'with-form' : ''}`}>
    ...
    {loading ? <div className="arcade-state">CARGANDO EQUIPOS...</div> : <div className="team-grid">{teams.map(team =>
      <button className="team-choice" key={team.id} onClick={() => onChoose(team.id)}><TeamMark team={team}/><span><b>{team.name}</b><small>PRESIDENTE: {team.president?.name ?? team.presidentName ?? 'SIN ASIGNAR'}</small><small>{team.playerCount ?? 0} JUGADORES · {team.currentDivision ?? 'LIGA ACTIVA'}</small></span><i>▶</i></button>)}</div>}
  </section></main>;
}
