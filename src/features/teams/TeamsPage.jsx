...
export function TeamsPage({ teams, loading, onChoose, onChanged, navigate }) {
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ name: '', presidentName: '', imageUrl: '', stadium: '', kind: 'club' });
  const mutation = useApiMutation((body, signal) => endpoints.createTeam(body, signal), {
    onSuccess: () => { setForm({ name: '', presidentName: '', imageUrl: '', stadium: '', kind: 'club' }); onChanged?.(); },
  });
  ...
  return <main className="newspaper teams-page"><section className={`team-list-paper ${showForm ? 'with-form' : ''}`}>
    ...
    {showForm && <form className="admin-form create-team-form" onSubmit={submit}><label>NOMBRE<input required name="name" value={form.name} onChange={change}/></label><label>PRESIDENTE<input name="presidentName" value={form.presidentName} onChange={change}/></label><label>ESTADIO<input name="stadium" maxLength="100" value={form.stadium} onChange={change}/></label><label>EMBLEMA URL<input type="url" name="imageUrl" value={form.imageUrl} onChange={change}/></label><label>TIPO<select name="kind" value={form.kind} onChange={change}><option value="club">CLUB</option><option value="national_team">SELECCIÓN</option></select></label><button className="action-button" disabled={mutation.loading}>{mutation.loading ? 'CREANDO...' : 'CREAR EQUIPO'}</button><FormFeedback mutation={mutation}/></form>}
