...
function RedCardDeleteButton({ matchId, redCard, onChanged }) {
  const mutation = useApiMutation(signal => endpoints.deleteMatchRedCard(matchId, redCard.id, signal), { onSuccess: onChanged });
  const remove = () => { if (window.confirm(`¿ANULAR LA ROJA DE ${redCard.playerName}? SE LEVANTA LA SUSPENSIÓN ASOCIADA.`)) mutation.execute(); };
  return <button className="danger" disabled={mutation.loading} onClick={remove}>ANULAR</button>;
}
