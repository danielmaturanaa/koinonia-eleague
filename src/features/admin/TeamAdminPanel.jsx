...
function SquadEditor({ team, squad, onChanged }) {
  ...
  const dropAt = (event, targetIndex) => {
    ...
    setOrdered(current => {
      ...
      next.splice(Math.min(targetIndex, next.length), 0, player);
      return next;
    });
    setDraggedId('');
  };
  ...
  const playerRow = (player, index) => <div className="squad-drag-row" key={player.id} onDragOver={event => event.preventDefault()} onDrop={event => dropAt(event, index)}>
    <button type="button" className="squad-drag-handle" draggable onDragStart={event => startDrag(event, player.id)} onDragEnd={() => setDraggedId('')} aria-label={`Mover a ${player.name}`}>↕</button>
    <b>{index + 1}</b><span><strong>{player.name}</strong><small>{player.position ?? 'SIN POSICIÓN'}</small></span>
    <label>DORSAL<input aria-label={`Dorsal de ${player.name}`} type="number" min="1" max="99" value={player.jerseyNumber ?? ''} onChange={event => updateJersey(player.id, event.target.value)}/></label>
  </div>;
  if (!squad.length) return <p className="admin-empty">NO HAY JUGADORES ASIGNADOS A ESTE EQUIPO.</p>;
  return <div className="squad-board"><p className="squad-help">ARRASTRA LOS JUGADORES ENTRE LOS PUESTOS. LAS POSICIONES 1 A 11 FORMAN EL EQUIPO TITULAR.</p><div className="squad-board-columns"><section><h3>TITULARES <small>11 PUESTOS</small></h3>{Array.from({ length: 11 }, (_, index) => ordered[index] ? playerRow(ordered[index], index) : <div className="squad-empty-slot" key={index} onDragOver={event => event.preventDefault()} onDrop={event => dropAt(event, index)}><b>{index + 1}</b><span>ARRASTRA UN JUGADOR AQUÍ</span></div>)}</section><section className="substitutes-drop-zone" onDragOver={event => event.preventDefault()} onDrop={event => dropAt(event, ordered.length)}><h3>SUPLENTES <small>{Math.max(ordered.length - 11, 0)}</small></h3>{ordered.slice(11).map((player, offset) => playerRow(player, offset + 11))}{ordered.length <= 11 && <div className="squad-empty-slot"><span>ARRASTRA AQUÍ PARA MOVER A SUPLENTES</span></div>}</section></div><button className="action-button squad-save-button" disabled={!ordered.length || mutation.loading} onClick={() => mutation.execute(ordered)}>GUARDAR PLANTEL COMPLETO</button><FormFeedback mutation={mutation}/></div>;
}

export function TeamAdminPanel({ team, squad, onChanged }) {
  const [tab, setTab] = useState('profile');
  const tabs = [['profile','PERFIL'],['president','PRESIDENTE'],['coach','DT'],['budget','PRESUPUESTO'],['squad','PLANTEL']];
