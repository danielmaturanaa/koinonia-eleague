...
function NewsDetailModal({ item, teams, onClose }) {
  if (!item) return null;
  return <div className="person-modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><article className="person-modal news-detail-modal" role="dialog" aria-modal="true" aria-label={item.headline}><header><h2>NOTICIA COMPLETA</h2><button type="button" onClick={onClose} aria-label="Cerrar">×</button></header><div className="news-detail-copy"><time>{item.publishedAt ? formatDate(item.publishedAt) : 'FECHA NO PUBLICADA'}</time><b>{item.label}</b><h2>{item.headline}</h2><NewsArtwork item={item} teams={teams}/>{item.subtitle && <h3>{item.subtitle}</h3>}<p>{item.body}</p></div></article></div>;
}
...
export function NewsPage({ teams = [] }) {
  ...
  useEffect(() => {
    ...
    if (!root || !sentinel || !hasMore || loading || loadingMore || moreError || error) return undefined;
