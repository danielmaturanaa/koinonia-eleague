...
export function NewsPage() {
  ...
  return <main className="newspaper data-page"><section className="data-paper"><PageHeader kicker="PERIÓDICO AUTOMÁTICO DE LA LIGA" title="NOTICIAS"/><DataState query={state}/>{!state.loading && !state.error && <div className="news-feed">{news.length ? news.map(item => <article className={`news-card news-${item.image}`} data-image={item.image} key={item.id}><div className="news-image-key" aria-hidden="true"><span>{item.sourceType === 'match' ? '⚽' : item.sourceType === 'transfer' ? '⇄' : '!'}</span></div><div><time>{item.date ? formatDate(item.date) : 'FECHA NO PUBLICADA'}</time><b>{item.label}</b><h2>{item.headline}</h2><p>{item.body}</p></div></article>) : <p className="empty-copy">NO HAY EVENTOS CON DATOS SUFICIENTES PARA GENERAR NOTICIAS.</p>}</div>}</section></main>;
}
