...
const normalizeSkillKey = key => String(key ?? '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '_');
export const skillLabel = key => SKILLS[key] ?? SKILLS[normalizeSkillKey(key)] ?? String(key ?? '').replaceAll('_', ' ');
