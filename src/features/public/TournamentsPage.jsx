...
function TournamentWorkspace({ tournamentId, classificationOnly, teams, onChanged, onDeleted }) {
  ...
  return <section className={`tournament-workspace ${classificationOnly ? 'classification-workspace' : 'management-workspace'}`}><DataState query={detail}/>{detail.data && <><header><div><small>{detail.data.format?.replaceAll('_', ' ')} · {detail.data.competitorKind}{detail.data.format === 'league' ? ` · CAMPEONES: ${detail.data.championPolicy === 'both' ? 'LIGA REGULAR + PLAYOFFS' : detail.data.championPolicy === 'playoffs' ? 'PLAYOFFS' : 'TABLA'}` : ''}</small><h2>{detail.data.name}</h2></div><b className={`status status-${detail.data.status}`}>{detail.data.status}</b></header>{classificationOnly ? <>
    <nav className="classification-view-tabs classification-content-tabs" aria-label="Datos del torneo"><button className={classificationView === 'standings' ? 'active' : ''} onClick={() => setClassificationView('standings')}>POSICIONES</button><button className={classificationView === 'scorers' ? 'active' : ''} onClick={() => setClassificationView('scorers')}>GOLEADORES</button>{detail.data.format === 'league' && <button className={classificationView === 'playoffs' ? 'active' : ''} onClick={() => setClassificationView('playoffs')}>PLAYOFFS</button>}</nav>
