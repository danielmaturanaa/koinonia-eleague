...
function CreateTournamentForm({ onChanged }) {
  ...
  return <form className="admin-form tournament-create-form" onSubmit={submit}><label>NOMBRE<input required name="name" value={form.name} onChange={change}/></label><label>FORMATO<select name="format" value={form.format} onChange={change}><option value="league">LIGA</option><option value="groups_knockout">GRUPOS + ELIMINACIÓN</option><option value="knockout">ELIMINACIÓN DIRECTA</option></select></label>{form.format === 'league' && <label>CAMPEÓN<select name="championPolicy" value={form.championPolicy} onChange={change}><option value="regular_season">TABLA DE FASE REGULAR</option><option value="playoffs">PLAYOFFS</option></select></label>}<label>COMPETIDORES<select name="competitorKind" value={form.competitorKind} onChange={change}><option value="club">CLUBES</option><option value="national_team">SELECCIONES</option></select></label><button className="action-button" disabled={mutation.loading}>CREAR TORNEO</button><FormFeedback mutation={mutation}/></form>;
}
...
function TournamentWorkspace({ tournamentId, classificationOnly, teams, onChanged, onDeleted }) {
  ...
  return <section className={`tournament-workspace ${classificationOnly ? 'classification-workspace' : 'management-workspace'}`}><DataState query={detail}/>{detail.data && <><header><div><small>{detail.data.format?.replaceAll('_', ' ')} · {detail.data.competitorKind}{detail.data.format === 'league' ? ` · CAMPEÓN POR ${detail.data.championPolicy === 'playoffs' ? 'PLAYOFFS' : 'TABLA'}` : ''}</small><h2>{detail.data.name}</h2></div><b className={`status status-${detail.data.status}`}>{detail.data.status}</b></header>{classificationOnly ? <>
...
export function TournamentsPage({ classificationOnly = false, teams = [] }) {
  ...
  return <main className="newspaper data-page"><section className="data-paper"><PageHeader kicker="COMPETICIONES OFICIALES" title={classificationOnly ? 'CLASIFICACIÓN' : 'TORNEOS'}>{classificationOnly ? <label className="classification-status-select">TORNEO<select value={statusFilter} onChange={event => setStatusFilter(event.target.value)} aria-label="Estado de los torneos"><option value="active">TORNEOS ACTIVOS</option><option value="completed">TORNEOS COMPLETADOS</option></select></label> : <button className="page-action" onClick={() => setShowCreate(value => !value)}>{showCreate ? 'CERRAR ALTA' : '+ CREAR TORNEO'}</button>}</PageHeader>{classificationOnly && <nav className="classification-view-tabs classification-status-tabs" aria-label="Estado de los torneos"><button className={statusFilter === 'active' ? 'active' : ''} onClick={() => setStatusFilter('active')}>TORNEOS ACTIVOS</button><button className={statusFilter === 'completed' ? 'active' : ''} onClick={() => setStatusFilter('completed')}>TORNEOS COMPLETADOS</button></nav>} {showCreate && <CreateTournamentForm onChanged={() => tournaments.retry()}/>}<DataState query={tournaments}/>{rows.length > 0 ? <div className="tournament-tabs">{rows.map(item => <button className={selectedId === item.id ? 'active' : ''} key={item.id} onClick={() => setSelectedId(item.id)}>{item.name}<small>{item.status}</small></button>)}</div> : !tournaments.loading && <p className="empty-copy">NO HAY TORNEOS {statusFilter === 'active' ? 'ACTIVOS' : 'COMPLETADOS'}.</p>}{selectedId && <TournamentWorkspace tournamentId={selectedId} classificationOnly={classificationOnly} teams={teams} onChanged={() => tournaments.retry()} onDeleted={deleted}/>}</section></main>;
}
