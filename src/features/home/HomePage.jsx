...
export function HomePage({ tournament, lead, teams = [] }) {
  ...
  return <main className="newspaper"><section className="main-edition">
    ...
    <article className="lead-story"><h2 className="automatic-news-headline"><span className="headline-team">{latest?.label ?? (loading ? 'ACTUALIZANDO' : 'KOINONIA e-LEAGUE')}</span><FittedHeadline>{latest?.headline ?? fallbackHeadline}</FittedHeadline></h2><div className="lead-columns"><div className="lead-copy">{latest ? <>{latest.subtitle && <h3 className="lead-subtitle">{latest.subtitle}</h3>}{latest.body && <p>{latest.body}</p>}{latest.body2 && <p className="lead-angle">{latest.body2}</p>}</> : <p>{fallbackBody}</p>}</div><figure className="lead-photo" role="img" aria-label="Portada deportiva de Koinonia e-League">{latest && <NewsArtwork item={latest} teams={teams}/>}</figure></div></article>
