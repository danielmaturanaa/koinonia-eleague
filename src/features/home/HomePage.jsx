...
function NewsModal({ item, teams, onClose }) {
  ...
  const GoalColumn = ({ label, team }) => <div className="news-acta-team"><small>{label}</small><strong>{team?.name ?? 'EQUIPO'}</strong><ul>{teamGoals(team).length ? teamGoals(team).map((goal, i) => <li key={goal.id ?? i}>{'⚽'.repeat(goal.count ?? 1)} {goal.playerName ?? goal.player?.name ?? 'Gol sin jugador'}</li>) : <li className="news-acta-empty">Sin goles registrados</li>}</ul></div>;
  return <div className="news-detail-backdrop" role="presentation" onClick={onClose}><article className="news-detail-modal" role="dialog" aria-modal="true" onClick={event => event.stopPropagation()}><button className="news-detail-close" onClick={onClose} aria-label="Cerrar noticia">×</button><NewsArtwork item={item} teams={teams}/><time>{item.publishedAt ?? item.date ? formatDate(item.publishedAt ?? item.date) : 'FECHA NO PUBLICADA'}</time><h2>{item.headline}</h2>{item.subtitle && <h3>{item.subtitle}</h3>}<p>{item.body}</p>{item.body2 && <p className="news-story-angle">{item.body2}</p>}{match && <section className="news-match-acta"><h3>ACTA DEL PARTIDO</h3><GoalColumn label="LOCAL" team={match.homeTeam}/><GoalColumn label="VISITA" team={match.awayTeam}/>{cards.length > 0 && <p>🟥 Sancionados: {cards.map(card => card.playerName ?? card.player?.name ?? 'Jugador').join(', ')}</p>}</section>}</article></div>;
}
