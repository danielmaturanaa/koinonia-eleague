...
function SecondaryNews({ item, teams }) {
  ...
  return <article className={`small-story automatic-small-story news-${item.image}`} data-image={item.image}><h3>{item.label}</h3><div className="secondary-news-content"><NewsArtwork item={item} teams={teams}/><div><time>{item.date ? formatDate(item.date) : 'FECHA NO PUBLICADA'}</time><h4>{item.headline}</h4><p>{item.body}</p></div></div></article>;
}
